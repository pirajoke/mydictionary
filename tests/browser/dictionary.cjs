...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator(".word-row").count(),8);
    assert.equal(await page.locator("#show-more-results").isVisible(),true);
    await page.locator("#show-more-results").click();
    assert.equal(await page.locator(".word-row").count(),16);
    ...
    assert.equal(await page.locator("#profile-saved").innerText(),"1");
    assert.equal(await page.locator("#profile-days").innerText(),"1");
    await page.evaluate(()=>{window.print=()=>{document.body.dataset.printed="true";};});
    await page.locator("#print-profile").click();
    assert.equal(await page.locator("body").getAttribute("data-printed"),"true");
    assert.equal(await page.locator("#view-saved").getAttribute("aria-pressed"),"true");
    ...
    assert(cacheUrls.every(url=>["/dictionary/","/static/dictionary-profile.js","/static/dictionary.css","/static/dictionary.js","/dictionary/manifest.webmanifest"].includes(url)));
    ...
    console.log(JSON.stringify({ok:true,checks:["42 language pairs","progressive results","Unicode/reverse lookup","saved profile persistence","print/PDF snapshot","written feedback","no external query traffic","mobile overflow","offline save/remove/resave","cold offline tab","standalone download","malformed storage"],output}));
