...
class MiniAppNavigationIllustrationsV2ContractTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        navigation = re.search(
            r'<nav\b(?=[^>]*\bclass="bottom-nav")[^>]*>.*?</nav>',
            cls.html,
            re.DOTALL,
        )
        if navigation is None:
            raise AssertionError("missing bottom navigation")
        cls.navigation = navigation.group(0)
    ...
    def test_ac1_each_tab_has_one_named_vector_illustration(self):
        ...
        self.assertEqual(self.navigation.count('viewBox="0 0 24 24"'), 5)
        for legacy_glyph in (">●</span>", ">◆</span>", ">★</span>", ">◎</span>", ">✦</span>"):
            with self.subTest(legacy_glyph=legacy_glyph):
                self.assertNotIn(legacy_glyph, self.navigation)

    def test_ac2_icons_share_a_richer_two_tone_badge_language(self):
        self.assertGreaterEqual(self.navigation.count('class="icon-soft"'), 5)
        for contract in (
            ".nav-glyph",
            ".icon-soft",
    ...
    def test_ac3_vectors_are_decorative_and_existing_tab_contract_survives(self):
        self.assertEqual(
            self.navigation.count('aria-hidden="true" focusable="false"'),
            5,
        )
        self.assertEqual(self.navigation.count('role="tab"'), 5)
